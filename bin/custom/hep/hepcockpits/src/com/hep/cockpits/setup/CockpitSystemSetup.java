 ...
 */
package com.hep.cockpits.setup;
...
import com.hep.cockpits.constants.HepCockpitsConstants;
 ...
 */
@SystemSetup(extension = HepCockpitsConstants.EXTENSIONNAME)
public class CockpitSystemSetup extends AbstractSystemSetup
{
	...
	public void createEssentialData(final SystemSetupContext context)
	{
		...
		{
			if (MediaManager.getInstance().getMediaFolderByQualifier(HepCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER).size() < 1)
			{
				MediaManager.getInstance().createMediaFolder(HepCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER,
						HepCockpitsConstants.JASPER_REPORTS_MEDIA_FOLDER);
			}
			try
			{
				String prefix = null;
				...
				if (prefix != null){
					importImpexFile(context, "/hepcockpits/reportcockpit/import/" + prefix + "_jasperreports.impex");
				}
			}
