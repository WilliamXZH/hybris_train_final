...
import com.hep.test.constants.HepTestConstants;
import com.hep.test.orders.AcceleratorTestOrderData;
 ...
 */
@SystemSetup(extension = HepTestConstants.EXTENSIONNAME)
public class TestDataSystemSetup extends AbstractSystemSetup
{
	...
	public void createProjectData(final SystemSetupContext context)
	{
		if (getBooleanSystemSetupParameter(context, CREATE_TEST_DATA))
		{
			...
			importImpexFile(context, "/heptest/import/qa-test-users.impex");

			LOG.info("Creating Reviews...");
			importImpexFile(context, "/heptest/import/reviews.impex");

			LOG.info("Creating Test Payment Subscriptions...");
			getAcceleratorTestOrderData().createPaymentInfos();

			LOG.info("Creating CS Tickets...");
			importImpexFile(context, "/heptest/import/cstickets.impex");
			...
			importImpexFile(context, "/heptest/import/csquotes.impex");
		}
	}
}
